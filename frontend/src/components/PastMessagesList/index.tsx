...
export default function PastMessagesList() {
  ...
  const sortedMessages = useMemo(() => {
    return messages?.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime())?.slice(0, 10)
  }, [messages])
  // #endregion
  ...
  }, [messagesDispatch, messagesService])
  // #endregion

  return (
    <DefaultBackground className="w-full gap-4 p-6 flex-none sm:flex-1 sm:h-full">
      <h1 className="text-2xl font-bold">Fila de mensagens</h1>
      <div className="flex flex-col w-full sm:flex-1 sm:overflow-hidden">
        {!sortedMessages && (
        ...
        {sortedMessages && (
          <>
            ...
            {sortedMessages.length > 0 && (
              <div className="flex flex-col gap-1 sm:overflow-auto px-2">
